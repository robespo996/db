USE [unscproduzione]
GO
/****** Object:  StoredProcedure [dbo].[SP_SIGMA_GENERAZIONE_LAVORAZIONE]    Script Date: 14/10/2025 12:36:30 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER PROCEDURE [dbo].[SP_SIGMA_GENERAZIONE_LAVORAZIONE] 
	(@CODICELOCALEMANDATO VARCHAR(50), @USERNAMERICHIESTA VARCHAR(50), @ESITO VARCHAR(255) OUTPUT)
AS
...
BEGIN TRY
	...
	IF EXISTS (SELECT * FROM sigma_generazione_file WHERE CODICELOCALEMANDATO = @CODICELOCALEMANDATO AND ISNULL(CARICATO,1) = 1)
	BEGIN
	...
	INSERT INTO sigma_generazione_file VALUES (@CODICELOCALEMANDATO,@Dataora,@USERNAMERICHIESTA,null,null,null,null)
	...
	select	distinct @IdGenerazione
			...
			,TEXT_CAUSALE_PAGAMENTO
			,ImportoLordo
	from [SQLDATI].gestionebanca.dbo.VW_MANDATI 
	union
	select	distinct @IdGenerazione
			,codiceMandato
			,codiceLocaleMandato 
			...
			,TEXT_CAUSALE_PAGAMENTO
			,ImportoLordo
	from [SQLDATI].gestionebanca.dbo.VW_MANDATI_FORMAZIONE_GG 	
	...
	select @IdGenerazione
			...
			,ANNO
			,MESE
	from [SQLDATI].gestionebanca.dbo.VW_MANDATI_VOLONTARIPAGATI 
	...
	select @IdGenerazione
			...
			,ANNO
			,MESE
	from [SQLDATI].gestionebanca.dbo.VW_REND_MANDATI_VOLONTARIPAGATI 
	...
	select	@IdGenerazione
			...
			,importo
			,importoRimborso
	from [SQLDATI].gestionebanca.dbo.VW_GG_PAGAMENTI_RIMBORSI 
	...
	select	@IdGenerazione
			...
			,DataValuta
			,codiceprogetto
	from [SQLDATI].gestionebanca.dbo.VW_PAGAMENTI_FORMAZIONE_GG_Progetti 

	--VERIFICO SE MANDATO RELATIVO A FORMAZIONE GENERALE
	IF EXISTS(SELECT DISTINCT codiceLocaleMandato 
				FROM SIGMA_SNAPSHOT_VW_PAGAMENTI_FORMAZIONE_GG_Progetti 
	...
	EXEC	[dbo].[SP_SIGMA_TABELLA_FN02_LAVORAZIONE] 
		@IdGenerazione
		,@CODICELOCALEMANDATO 
		,@EsitoFN02 OUTPUT
	...
	EXEC	[dbo].[SP_SIGMA_TABELLA_FN06_LAVORAZIONE] 
		@IdGenerazione
		,@CODICELOCALEMANDATO 
		,@EsitoFN06 OUTPUT
	...
	EXEC	[dbo].[SP_SIGMA_TABELLA_FN08_LAVORAZIONE] 
		@IdGenerazione
		,@CODICELOCALEMANDATO 
		,@EsitoFN08 OUTPUT
	...
	EXEC	[dbo].[SP_SIGMA_TABELLA_SD01_LAVORAZIONE] 
		@IdGenerazione
		,@CODICELOCALEMANDATO 
		,@EsitoSD01 OUTPUT	
	...
	EXEC	[dbo].[SP_SIGMA_TABELLA_SD02_LAVORAZIONE]
		@IdGenerazione
		,@CODICELOCALEMANDATO 
		,@EsitoSD02 OUTPUT	
	...
	EXEC	[dbo].[SP_SIGMA_TABELLA_SD03_LAVORAZIONE] 
		@IdGenerazione
		,@CODICELOCALEMANDATO 
		,@EsitoSD03 OUTPUT	
	...
	EXEC	[dbo].[SP_SIGMA_TABELLA_SD04_LAVORAZIONE] 
		@IdGenerazione
		,@CODICELOCALEMANDATO 
		,@EsitoSD04 OUTPUT	
	...
	EXEC	[dbo].[SP_SIGMA_TABELLA_SD06_LAVORAZIONE]
		@IdGenerazione
		,@CODICELOCALEMANDATO 
		,@EsitoSD06 OUTPUT	
	...
	IF ISNULL(@ESITO,'') = ''
		BEGIN
			...
			BEGIN			
				...
				exec master..xp_cmdshell @comandobcp
					...
					SELECT 
						...
						,''' + @USERNAMERICHIESTA + '''
					FROM OPENROWSET(BULK ''\\Appl\modhelios$\SIGMA_FILES\' + @nomefile +''' ,SINGLE_BLOB ) AS x '
					EXEC (@SQL) 	
				...
				--dati
				set @sql = @sql + ' UNION SELECT nullif([COD_LOCALE_PROGETTO],''''),nullif([TITOLO_PROGETTO],''''),nullif([SINTESI_PRG],''''),nullif([COD_MISURA],''''),nullif([TIPO_OPERAZIONE],''''),nullif([CUP],''''),nullif([TIPO_AIUTO],''''),nullif([DATA INIZIO],''''),nullif([DATA FINE PREVISTA],''''),nullif([DATA FINE EFFETTIVA],''''),nullif([COD_PROC_ATT_LOCALE],'''') ,nullif([NOTE_PROGETTO],''''),nullif([FLG_CANCELLAZIONE],''''),idgenerazione FROM unscproduzione.dbo.SIGMA_TABELLA_AP00 where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY idgenerazione,COD_LOCALE_PROGETTO'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\CSV\' + @nomefile + ' -c -t; -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp

					--upload
					set @sql = 'UPDATE SIGMA_Generazione_File_Allegati 
									SET FileNameCSV = ''' + @nomefile + ''', BINDATACSV = CAST(bulkcolumn AS VARBINARY(MAX)) 
								FROM OPENROWSET(BULK ''\\Appl\modhelios$\SIGMA_FILES\CSV\' + @nomefile + ''' ,SINGLE_BLOB ) AS x 
			...
			BEGIN			
				...
				set @sql = 'SELECT nullif([COD_LOCALE_PROGETTO],''''),nullif([GENERATORE_ENTRATE],''''),nullif([LIV_ISTRUZIONE_STR_FIN],''''),nullif([FONDO_DI_FONDI],''''),nullif([TIPO_LOCALIZZAZIONE],''''),nullif([COD_VULNERABILI],''''),nullif([FLG_CANCELLAZIONE],'''') FROM unscproduzione.dbo.SIGMA_TABELLA_AP02 where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY COD_LOCALE_PROGETTO'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\' + @nomefile + ' -c -t"|" -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp
				...
				--dati
				set @sql = @sql + ' UNION SELECT nullif([COD_LOCALE_PROGETTO],''''),nullif([GENERATORE_ENTRATE],''''),nullif([LIV_ISTRUZIONE_STR_FIN],''''),nullif([FONDO_DI_FONDI],''''),nullif([TIPO_LOCALIZZAZIONE],''''),nullif([COD_VULNERABILI],''''),nullif([FLG_CANCELLAZIONE],''''),idgenerazione FROM unscproduzione.dbo.SIGMA_TABELLA_AP02 where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY idgenerazione,COD_LOCALE_PROGETTO'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\CSV\' + @nomefile + ' -c -t; -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp
			...
			BEGIN			
				...
				set @sql = 'SELECT nullif([COD_LOCALE_PROGETTO],'''') ,nullif([TIPO_CLASS],'''') ,nullif([COD_CLASSIFICAZIONE],'''') ,nullif([FLG_CANCELLAZIONE],'''') FROM unscproduzione.dbo.SIGMA_TABELLA_AP03 where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY COD_LOCALE_PROGETTO'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\' + @nomefile + ' -c -t"|" -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp
				...
				--dati
				set @sql = @sql + ' UNION SELECT nullif([COD_LOCALE_PROGETTO],'''') ,nullif([TIPO_CLASS],'''') ,nullif([COD_CLASSIFICAZIONE],'''') ,nullif([FLG_CANCELLAZIONE],''''),idgenerazione FROM unscproduzione.dbo.SIGMA_TABELLA_AP03 where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY idgenerazione,COD_LOCALE_PROGETTO'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\CSV\' + @nomefile + ' -c -t; -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp
			...
			BEGIN
				...
				set @sql = 'SELECT nullif([COD_LOCALE_PROGETTO],''''),nullif([COD_REGIONE],''''),nullif([COD_PROVINCIA],''''),nullif([COD_COMUNE],''''),nullif([INDIRIZZO],''''),nullif([COD_CAP],''''),nullif([FLG_CANCELLAZIONE],'''') FROM unscproduzione.dbo.SIGMA_TABELLA_AP06 where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY COD_LOCALE_PROGETTO'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\' + @nomefile + ' -c -t"|" -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp		
				...
				--dati
				set @sql = @sql + ' UNION SELECT nullif([COD_LOCALE_PROGETTO],''''),nullif([COD_REGIONE],''''),nullif([COD_PROVINCIA],''''),nullif([COD_COMUNE],''''),nullif([INDIRIZZO],''''),nullif([COD_CAP],''''),nullif([FLG_CANCELLAZIONE],''''),idgenerazione FROM unscproduzione.dbo.SIGMA_TABELLA_AP06 where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY idgenerazione,COD_LOCALE_PROGETTO'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\CSV\' + @nomefile + ' -c -t; -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp	
			...
			BEGIN
				...
				set @sql = 'SELECT nullif([COD_LOCALE_PROGETTO],''''),nullif([COD_MISURA],''''),nullif([DATA_IMPEGNO],''''),nullif([COD_IMPEGNO],''''),nullif([TIPOLOGIA_IMPEGNO],''''),nullif([COD_FONDO],''''),nullif([COD_NORMA],''''),nullif([COD_DEL_CIPE],''''),nullif([COD_LOCALIZZAZIONE],''''),nullif([CF_COFINANZ],''''),nullif([IMPORTO],''''),nullif([FLG_CANCELLAZIONE],'''') FROM unscproduzione.dbo.SIGMA_TABELLA_FN00 where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY COD_LOCALE_PROGETTO,COD_FONDO'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\' + @nomefile + ' -c -t"|" -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp		
				...
				--dati
				set @sql = @sql + ' UNION SELECT nullif([COD_LOCALE_PROGETTO],''''),nullif([COD_MISURA],''''),nullif([DATA_IMPEGNO],''''),nullif([COD_IMPEGNO],''''),nullif([TIPOLOGIA_IMPEGNO],''''),nullif([COD_FONDO],''''),nullif([COD_NORMA],''''),nullif([COD_DEL_CIPE],''''),nullif([COD_LOCALIZZAZIONE],''''),nullif([CF_COFINANZ],''''),nullif([IMPORTO],''''),nullif([FLG_CANCELLAZIONE],''''),idgenerazione FROM unscproduzione.dbo.SIGMA_TABELLA_FN00 where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY idgenerazione,COD_LOCALE_PROGETTO,COD_FONDO'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\CSV\' + @nomefile + ' -c -t; -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp	
			...
			--SEMPRE, ANCHE PER MANDATO FORMAZIONE
				...
				set @sql = 'SELECT nullif([COD_LOCALE_PROGETTO],'''') ,nullif([VOCE_SPESA],'''') ,nullif([IMPORTO],'''') ,nullif([FLG_CANCELLAZIONE],'''') FROM unscproduzione.dbo.SIGMA_TABELLA_FN02_LAVORAZIONE where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY COD_LOCALE_PROGETTO, IMPORTO'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\' + @nomefile + ' -c -t"|" -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp	
				...
				--dati
				set @sql = @sql + ' UNION SELECT nullif([COD_LOCALE_PROGETTO],'''') ,nullif([VOCE_SPESA],'''') ,nullif([IMPORTO],'''') ,nullif([FLG_CANCELLAZIONE],''''),idgenerazione FROM unscproduzione.dbo.SIGMA_TABELLA_FN02_LAVORAZIONE where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY idgenerazione,COD_LOCALE_PROGETTO, IMPORTO'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\CSV\' + @nomefile + ' -c -t; -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp	
			...
			--SEMPRE, ANCHE PER MANDATO FORMAZIONE
				...
				set @sql = 'SELECT nullif([COD_LOCALE_PROGETTO],'''') ,nullif([COD_PAGAMENTO],'''') ,nullif([TIPOLOGIA_PAG],'''') ,nullif([DATA_PAGAMENTO],'''') ,nullif([COD_LOCALE_DDR],'''') ,nullif([DATA_COMPETENZA_DAL],'''') ,nullif([DATA_COMPETENZA_AL],'''') ,nullif([IMPORTO_PAG],'''') ,nullif([CAUSALE_PAGAMENTO],'''') ,nullif([NOTE_PAG],''''),nullif([FLG_CANCELLAZIONE],'''') FROM unscproduzione.dbo.SIGMA_TABELLA_FN06_LAVORAZIONE where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY COD_LOCALE_PROGETTO,COD_PAGAMENTO'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\' + @nomefile + ' -c -t"|" -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp
				...
				--dati
				set @sql = @sql + ' UNION SELECT nullif([COD_LOCALE_PROGETTO],'''') ,nullif([COD_PAGAMENTO],'''') ,nullif([TIPOLOGIA_PAG],'''') ,nullif([DATA_PAGAMENTO],'''') ,nullif([COD_LOCALE_DDR],'''') ,nullif([DATA_COMPETENZA_DAL],'''') ,nullif([DATA_COMPETENZA_AL],'''') ,nullif([IMPORTO_PAG],'''') ,nullif([CAUSALE_PAGAMENTO],'''') ,nullif([NOTE_PAG],''''),nullif([FLG_CANCELLAZIONE],''''),idgenerazione FROM unscproduzione.dbo.SIGMA_TABELLA_FN06_LAVORAZIONE where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY idgenerazione,COD_LOCALE_PROGETTO,COD_PAGAMENTO'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\CSV\' + @nomefile + ' -c -t; -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp
			...
			--SEMPRE, ANCHE PER MANDATO FORMAZIONE
				...
				set @sql = 'SELECT nullif([COD_LOCALE_PROGETTO],''''),nullif([COD_PAGAMENTO],''''),nullif([TIPOLOGIA_PAG],'''') ,nullif([DATA_PAGAMENTO],'''') ,nullif([CODICE_FISCALE],'''') ,nullif([FLAG_SOGGETTO_PUBBLICO],'''') ,nullif([TIPO_PERCETTORE],'''') ,nullif([IMPORTO],'''') ,nullif([FLG_CANCELLAZIONE],'''') FROM unscproduzione.dbo.SIGMA_TABELLA_FN08_LAVORAZIONE where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY COD_LOCALE_PROGETTO,COD_PAGAMENTO,DATA_PAGAMENTO,CODICE_FISCALE'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\' + @nomefile + ' -c -t"|" -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp
				...
				--dati
				set @sql = @sql + ' UNION SELECT nullif([COD_LOCALE_PROGETTO],''''),nullif([COD_PAGAMENTO],''''),nullif([TIPOLOGIA_PAG],'''') ,nullif([DATA_PAGAMENTO],'''') ,nullif([CODICE_FISCALE],'''') ,nullif([FLAG_SOGGETTO_PUBBLICO],'''') ,nullif([TIPO_PERCETTORE],'''') ,nullif([IMPORTO],'''') ,nullif([FLG_CANCELLAZIONE],''''),idgenerazione FROM unscproduzione.dbo.SIGMA_TABELLA_FN08_LAVORAZIONE where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY idgenerazione,COD_LOCALE_PROGETTO,COD_PAGAMENTO,DATA_PAGAMENTO,CODICE_FISCALE'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\CSV\' + @nomefile + ' -c -t; -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp
			...
			BEGIN
				...
				set @sql = 'SELECT nullif([COD_LOCALE_PROGETTO],''''),nullif([COD_CORSO],'''') ,nullif([INDICE_ANNUALITA],'''') ,nullif([NUMERO_ANNUALITA],'''') ,nullif([TITOLO_CORSO],'''') ,nullif([COD_MODALITA_FORMATIVA],'''') ,nullif([COD_CONTENUTO_FORMATIVO],'''') ,nullif([DATA_AVVIO],'''') ,nullif([DATA_CONCLUSIONE],'''') ,nullif([COD_CRITERI_SELEZIONE],'''') ,nullif([ESAME_FINALE],'''') ,nullif([COD_ATTESTAZIONE_FINALE],'''') ,nullif([COD_QUALIFICA],'''') ,nullif([STAGE_TIROCINI] ,''''),nullif([DURATA_AULA],'''') ,nullif([DURATA_WE],'''') ,nullif([DURATA_LABORATORIO],'''') ,nullif([DOCENTI_TUTOR],'''') ,nullif([FLAG_VOUCHER],'''') ,nullif([FLG_CANCELLAZIONE],'''') FROM unscproduzione.dbo.SIGMA_TABELLA_FO00 where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY COD_LOCALE_PROGETTO'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\' + @nomefile + ' -c -t"|" -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp	
				...
				--dati
				set @sql = @sql + ' UNION SELECT nullif([COD_LOCALE_PROGETTO],''''),nullif([COD_CORSO],'''') ,nullif([INDICE_ANNUALITA],'''') ,nullif([NUMERO_ANNUALITA],'''') ,nullif([TITOLO_CORSO],'''') ,nullif([COD_MODALITA_FORMATIVA],'''') ,nullif([COD_CONTENUTO_FORMATIVO],'''') ,nullif([DATA_AVVIO],'''') ,nullif([DATA_CONCLUSIONE],'''') ,nullif([COD_CRITERI_SELEZIONE],'''') ,nullif([ESAME_FINALE],'''') ,nullif([COD_ATTESTAZIONE_FINALE],'''') ,nullif([COD_QUALIFICA],'''') ,nullif([STAGE_TIROCINI] ,''''),nullif([DURATA_AULA],'''') ,nullif([DURATA_WE],'''') ,nullif([DURATA_LABORATORIO],'''') ,nullif([DOCENTI_TUTOR],'''') ,nullif([FLAG_VOUCHER],'''') ,nullif([FLG_CANCELLAZIONE],''''),idgenerazione FROM unscproduzione.dbo.SIGMA_TABELLA_FO00 where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY idgenerazione,COD_LOCALE_PROGETTO'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\CSV\' + @nomefile + ' -c -t; -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp	
			...
			BEGIN
				...
				set @sql = 'SELECT nullif([COD_LOCALE_PROGETTO],'''') ,nullif([COD_FASE],'''') ,nullif([DATA_INIZIO_PREVISTA],'''') ,nullif([DATA_INIZIO_EFFETTIVA] ,''''),nullif([DATA_FINE_PREVISTA],'''') ,nullif([DATA_FINE_EFFETTIVA],'''') ,nullif([FLG_CANCELLAZIONE],'''') FROM unscproduzione.dbo.SIGMA_TABELLA_PR00 where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY COD_LOCALE_PROGETTO'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\' + @nomefile + ' -c -t"|" -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp
				...
				--dati
				set @sql = @sql + ' UNION SELECT nullif([COD_LOCALE_PROGETTO],'''') ,nullif([COD_FASE],'''') ,nullif([DATA_INIZIO_PREVISTA],'''') ,nullif([DATA_INIZIO_EFFETTIVA] ,''''),nullif([DATA_FINE_PREVISTA],'''') ,nullif([DATA_FINE_EFFETTIVA],'''') ,nullif([FLG_CANCELLAZIONE],''''),idgenerazione FROM unscproduzione.dbo.SIGMA_TABELLA_PR00 where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY idgenerazione,COD_LOCALE_PROGETTO'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\CSV\' + @nomefile + ' -c -t; -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp
			...
			BEGIN
				...
				set @sql = 'SELECT nullif([COD_LOCALE_PROGETTO],'''') ,nullif([STATO_PROGETTO],'''') ,nullif([DATA_RIFERIMENTO],'''') ,nullif([FLG_CANCELLAZIONE],'''') FROM unscproduzione.dbo.SIGMA_TABELLA_PR01 where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY COD_LOCALE_PROGETTO'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\' + @nomefile + ' -c -t"|" -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp
				...
				--dati
				set @sql = @sql + ' UNION SELECT nullif([COD_LOCALE_PROGETTO],'''') ,nullif([STATO_PROGETTO],'''') ,nullif([DATA_RIFERIMENTO],'''') ,nullif([FLG_CANCELLAZIONE],''''),idgenerazione FROM unscproduzione.dbo.SIGMA_TABELLA_PR01 where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY idgenerazione,COD_LOCALE_PROGETTO'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\CSV\' + @nomefile + ' -c -t; -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp
			...
			BEGIN
				...
				--dati
				set @sql = @sql + ' UNION SELECT nullif([COD_LOCALE_PROGETTO],''''),nullif([CODICE_CORSO],'''') ,nullif([CODICE_FISCALE] ,''''),nullif([SESSO],'''') ,nullif([DATA_NASCITA],'''') ,nullif([COD_ISTAT_RES] ,''''),nullif([COD_ISTAT_DOM],'''') ,nullif([CITTADINANZA],'''') ,nullif([TITOLO_STUDIO],'''') ,nullif([COND_MERCATO_INGRESSO],'''') ,nullif([DURATA_RICERCA],'''') ,nullif([CODICE_VULNERABILE_PA] ,''''),nullif([STATO_PARTECIPANTE],'''') ,nullif([DATA_USCITA],'''') ,nullif([TIPO_LAVORO],'''') ,nullif([FLG_CANCELLAZIONE],''''),idgenerazione 	FROM unscproduzione.dbo.SIGMA_TABELLA_SC01 where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY idgenerazione,COD_LOCALE_PROGETTO,CODICE_FISCALE'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\CSV\' + @nomefile + ' -c -t; -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp
			...
			--SEMPRE, ANCHE PER MANDATO FORMAZIONE
				...
				set @sql = 'SELECT nullif([COD_LOCALE_MANDATO],'''') ,nullif([NUMERO_MANDATO],'''') ,nullif([DATA_MANDATO],'''') ,nullif([IMPORTO_MANDATO],'''') ,nullif([NOTE],'''') FROM unscproduzione.dbo.SIGMA_TABELLA_SD02_LAVORAZIONE where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY COD_LOCALE_MANDATO'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\' + @nomefile + ' -c -t"|" -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp
				...
				--dati
				set @sql = @sql + ' UNION SELECT nullif([COD_LOCALE_MANDATO],'''') ,nullif([NUMERO_MANDATO],'''') ,nullif([DATA_MANDATO],'''') ,nullif([IMPORTO_MANDATO],'''') ,nullif([NOTE],''''),idgenerazione FROM unscproduzione.dbo.SIGMA_TABELLA_SD02_LAVORAZIONE where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY idgenerazione,COD_LOCALE_MANDATO'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\CSV\' + @nomefile + ' -c -t; -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp
			...
			--SEMPRE, ANCHE PER MANDATO FORMAZIONE
				...
				--dati
				set @sql = @sql + ' UNION SELECT nullif([COD_LOCALE_DOCUMENTO],'''') ,nullif([NOME_FILE],'''') ,nullif([TIPOLOGIA_FILE],'''') ,nullif([CATEGORIA_FILE],'''') ,nullif([COD_LOCALE_PROGETTO],'''') ,nullif([COD_LOCALE_MANDATO],'''') ,nullif([COD_SPESA_LOCALE],'''') ,nullif([NOTE],''''),idgenerazione FROM unscproduzione.dbo.SIGMA_TABELLA_SD03_LAVORAZIONE where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY idgenerazione,COD_LOCALE_DOCUMENTO'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\CSV\' + @nomefile + ' -c -t; -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp
			...
			--SEMPRE, ANCHE PER MANDATO FORMAZIONE
				...
				set @sql = 'SELECT nullif([COD_LOCALE_MANDATO],''''),nullif([COD_LOCALE_SPESA],'''') FROM unscproduzione.dbo.SIGMA_TABELLA_SD04_LAVORAZIONE where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY COD_LOCALE_MANDATO,COD_LOCALE_SPESA'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\' + @nomefile + ' -c -t"|" -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp
				...
				--dati
				set @sql = @sql + ' UNION SELECT nullif([COD_LOCALE_MANDATO],''''),nullif([COD_LOCALE_SPESA],''''),idgenerazione FROM unscproduzione.dbo.SIGMA_TABELLA_SD04_LAVORAZIONE where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY idgenerazione,COD_LOCALE_MANDATO,COD_LOCALE_SPESA'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\CSV\' + @nomefile + ' -c -t; -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp
			...
			--SEMPRE, ANCHE PER MANDATO FORMAZIONE
				...
				set @sql = 'SELECT nullif([COD_LOCALE_SPESA],''''),nullif([COD_PAGAMENTO],'''') FROM unscproduzione.dbo.SIGMA_TABELLA_SD06_LAVORAZIONE where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY COD_LOCALE_SPESA, COD_PAGAMENTO'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\' + @nomefile + ' -c -t"|" -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp	
				...
				--dati
				set @sql = @sql + ' UNION SELECT nullif([COD_LOCALE_SPESA],''''),nullif([COD_PAGAMENTO],''''),idgenerazione FROM unscproduzione.dbo.SIGMA_TABELLA_SD06_LAVORAZIONE where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY idgenerazione,COD_LOCALE_SPESA, COD_PAGAMENTO'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\CSV\' + @nomefile + ' -c -t; -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp	
