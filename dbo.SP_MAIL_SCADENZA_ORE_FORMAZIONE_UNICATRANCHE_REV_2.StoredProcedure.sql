USE [unscproduzione]
GO
/****** Object:  StoredProcedure [dbo].[SP_MAIL_SCADENZA_ORE_FORMAZIONE_UNICATRANCHE_REV_2]    Script Date: 14/10/2025 12:36:28 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[SP_MAIL_SCADENZA_ORE_FORMAZIONE_UNICATRANCHE_REV_2]
...
SET @CONTA = 0
SET @OGGETTO = 'AVVISO SCADENZA ORE DI FORMAZIONE (UNICA TRANCHE)'
	...
	DECLARE MYCUR CURSOR LOCAL FOR
			SELECT attivit‡.IDAttivit‡, REGIONICOMPETENZE.DESCRIZIONE AS COMPETENZA, ENTI.DENOMINAZIONE + ' (' + enti.codiceregione + ')' as Ente, attivit‡.TITOLO + ' (' + attivit‡.CODICEENTE + ')' as Progetto, 
				...
				(select count(distinct a.identit‡) 
					...
					and dbo.formatodatadt(e.datascadenzaunicatranche) between a.datainizioservizio and a.datafineservizio) as Volontari150,
				 (select  count(distinct a.identit‡) 
					from entit‡ a 
					inner join attivit‡entit‡ b on a.identit‡ = b.identit‡
					inner join attivit‡entisediattuazione c on b.idattivit‡entesedeattuazione = c.idattivit‡entesedeattuazione
			...
			AND ATTIVIT‡FORMAZIONEGENERALE.TIPOFORMAZIONEGENERALE = 1	
			AND ATTIVIT‡.DATAINIZIOATTIVIT‡ IS NOT NULL and BANDO.idbando not in (549,550,551,553)---aggiunta per proroga titol	
