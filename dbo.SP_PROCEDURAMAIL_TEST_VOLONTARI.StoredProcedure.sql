USE [unscproduzione]
GO
/****** Object:  StoredProcedure [dbo].[SP_PROCEDURAMAIL_TEST_VOLONTARI]    Script Date: 14/10/2025 12:36:29 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[SP_PROCEDURAMAIL_TEST_VOLONTARI]
--@gruppo as int
AS
DECLARE
            @OGGETTO          VARCHAR(500),
...
'
      ...
      DECLARE MYCUR CURSOR LOCAL FOR
            select top 10 codicevolontario,EMAIL, gruppo from [unscproduzione].[dbo]._Mail_TEST_VOLONTARI
			where gruppo > 0
      ...
      BEGIN
			...
			PRINT 'VOLONTARIO: ' + @codicevolontario + ' ' + @mail + ' (' + convert(varchar, @gruppo) + ')'
            --EXEC @RETURN = [SP_INVIO_MAIL_COMUNICAZIONE] '','',@MAIL,'','',@OGGETTO ,@TESTO,'d:\allegati\Nota_Capo_Dipartimento.pdf;d:\allegati\Nota_Ministro_Dadone.pdf'
            FETCH NEXT FROM MYCUR INTO @CODICEvolontario, @MAIL, @gruppo
...
DEALLOCATE MYCUR
			...
			UPDATE [unscproduzione].[dbo]._Mail_TEST_VOLONTARI
			   SET gruppo = 0
			 WHERE gruppo = @gruppo
			 PRINT 'AGGIORNATO GRUPPO: ' + convert(varchar, @gruppo)

SET @OGGETTO = @OGGETTO + ' - CONCLUSIONE INVIO'
--EXEC @RETURN = [SP_INVIO_MAIL_COMUNICAZIONE] '','','mpetracca@serviziocivile.it','','',@OGGETTO ,@TESTO,'d:\allegati\Nota_Capo_Dipartimento.pdf;d:\allegati\Nota_Ministro_Dadone.pdf'
