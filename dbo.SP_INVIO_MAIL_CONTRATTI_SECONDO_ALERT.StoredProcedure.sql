USE [unscproduzione]
GO
/****** Object:  StoredProcedure [dbo].[SP_INVIO_MAIL_CONTRATTI_SECONDO_ALERT]    Script Date: 14/10/2025 12:36:28 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
-- Procedura Invio Mail relativo al giorno 10 del mese successivo alla data di inizio servizio
ALTER PROCEDURE [dbo].[SP_INVIO_MAIL_CONTRATTI_SECONDO_ALERT]
AS
	DECLARE
	...
	BEGIN
		...
		AND     convert(date,GETDATE())=DATEADD(mm,1,CONVERT(date,CONVERT(varchar,CONVERT(varchar,YEAR(Entit‡.datainizioservizio))+'/'+CONVERT(varchar,MONTH	(Entit‡.datainizioservizio))+'/'+'10',105)))
		AND     (Entit‡.datafineservizio is null or (convert(date,Entit‡.datainizioservizio)<>convert(date,Entit‡.datafineservizio)))
		AND	Entit‡.datainizioservizio >= CONVERT(date,'2024-05-28')
		...
		BEGIN
			...
			SET @PRIMAVOLTA = 1
			SET @IDALERT = 0 
			SET @OGGETTO = 'ENTE ' + @ENTESUFFISSO + ' - AVVISO MANCATO CARICAMENTO A SISTEMA DEI DOCUMENTI AFFERENTI GLI O.V. AVVIATI AL SERVIZIO - SECONDO ALERT'
			SET @MESSAGGIO = '<FONT face="Verdana" size="4">Si segnala nuovamente che, con riferimento agli O.V. indicati in tabella, non risulta il caricamento nel Sistema Unico del contratto.<br><br>
...
Si rammenta che il mancato caricamento del contratto comporta la non corresponsione dell''assegno mensile al singolo operatore volontario.
</FONT><br><br>
			...
			BEGIN
				...
				BEGIN
					...
					VALUES (GETDATE(), @IDENTE, @ENTESUFFISSO, @@CURSOR_ROWS,'SECONDO ALERT')
	...
	IF @RUNESEGUITO = 0 
		BEGIN
			...
			VALUES (GETDATE(), 'Nessuna occorrenza trovata per il run corrente','SECONDO ALERT')
		END
	ELSE
		BEGIN
			INSERT INTO LOG_ALERT_RUN (DataRun, Esito, TipoAlert)
			VALUES (GETDATE(), 'Trovata almeno una occorrenza il run corrente. Consultare tabelle SUMMARY e DETAIL','SECONDO ALERT')
		END
GO
