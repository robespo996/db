USE [unscproduzione]
GO
/****** Object:  StoredProcedure [dbo].[SP_SIGMA_GENERAZIONE_BACKUP]    Script Date: 14/10/2025 12:36:30 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[SP_SIGMA_GENERAZIONE_BACKUP] 
	...
	IF ISNULL(@ESITO,'') = ''
		BEGIN
			...
			--FN02
				...
				set @sql = 'SELECT nullif([COD_LOCALE_PROGETTO],'''') ,nullif([VOCE_SPESA],'''') ,nullif([IMPORTO],'''') ,nullif([FLG_CANCELLAZIONE],'''') FROM unscproduzione.dbo.SIGMA_TABELLA_FN02 where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY COD_LOCALE_PROGETTO, IMPORTO'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\' + @nomefile + ' -c -t"|" -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp	
				...
				--dati
				set @sql = @sql + ' UNION SELECT nullif([COD_LOCALE_PROGETTO],'''') ,nullif([VOCE_SPESA],'''') ,nullif([IMPORTO],'''') ,nullif([FLG_CANCELLAZIONE],''''),idgenerazione FROM unscproduzione.dbo.SIGMA_TABELLA_FN02 where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY idgenerazione,COD_LOCALE_PROGETTO, IMPORTO'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\CSV\' + @nomefile + ' -c -t; -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp	
			...
			--FN06
				...
				set @sql = 'SELECT nullif([COD_LOCALE_PROGETTO],'''') ,nullif([COD_PAGAMENTO],'''') ,nullif([TIPOLOGIA_PAG],'''') ,nullif([DATA_PAGAMENTO],'''') ,nullif([COD_LOCALE_DDR],'''') ,nullif([DATA_COMPETENZA_DAL],'''') ,nullif([DATA_COMPETENZA_AL],'''') ,nullif([IMPORTO_PAG],'''') ,nullif([CAUSALE_PAGAMENTO],'''') ,nullif([NOTE_PAG],''''),nullif([FLG_CANCELLAZIONE],'''') FROM unscproduzione.dbo.SIGMA_TABELLA_FN06 where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY COD_LOCALE_PROGETTO,COD_PAGAMENTO'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\' + @nomefile + ' -c -t"|" -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp
				...
				--dati
				set @sql = @sql + ' UNION SELECT nullif([COD_LOCALE_PROGETTO],'''') ,nullif([COD_PAGAMENTO],'''') ,nullif([TIPOLOGIA_PAG],'''') ,nullif([DATA_PAGAMENTO],'''') ,nullif([COD_LOCALE_DDR],'''') ,nullif([DATA_COMPETENZA_DAL],'''') ,nullif([DATA_COMPETENZA_AL],'''') ,nullif([IMPORTO_PAG],'''') ,nullif([CAUSALE_PAGAMENTO],'''') ,nullif([NOTE_PAG],''''),nullif([FLG_CANCELLAZIONE],''''),idgenerazione FROM unscproduzione.dbo.SIGMA_TABELLA_FN06 where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY idgenerazione,COD_LOCALE_PROGETTO,COD_PAGAMENTO'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\CSV\' + @nomefile + ' -c -t; -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp
			...
			--FN08
				...
				set @sql = 'SELECT nullif([COD_LOCALE_PROGETTO],''''),nullif([COD_PAGAMENTO],''''),nullif([TIPOLOGIA_PAG],'''') ,nullif([DATA_PAGAMENTO],'''') ,nullif([CODICE_FISCALE],'''') ,nullif([FLAG_SOGGETTO_PUBBLICO],'''') ,nullif([TIPO_PERCETTORE],'''') ,nullif([IMPORTO],'''') ,nullif([FLG_CANCELLAZIONE],'''') FROM unscproduzione.dbo.SIGMA_TABELLA_FN08 where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY COD_LOCALE_PROGETTO,COD_PAGAMENTO,DATA_PAGAMENTO,CODICE_FISCALE'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\' + @nomefile + ' -c -t"|" -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp
				...
				--dati
				set @sql = @sql + ' UNION SELECT nullif([COD_LOCALE_PROGETTO],''''),nullif([COD_PAGAMENTO],''''),nullif([TIPOLOGIA_PAG],'''') ,nullif([DATA_PAGAMENTO],'''') ,nullif([CODICE_FISCALE],'''') ,nullif([FLAG_SOGGETTO_PUBBLICO],'''') ,nullif([TIPO_PERCETTORE],'''') ,nullif([IMPORTO],'''') ,nullif([FLG_CANCELLAZIONE],''''),idgenerazione FROM unscproduzione.dbo.SIGMA_TABELLA_FN08 where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY idgenerazione,COD_LOCALE_PROGETTO,COD_PAGAMENTO,DATA_PAGAMENTO,CODICE_FISCALE'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\CSV\' + @nomefile + ' -c -t; -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp
			...
			--SD02
				...
				set @sql = 'SELECT nullif([COD_LOCALE_MANDATO],'''') ,nullif([NUMERO_MANDATO],'''') ,nullif([DATA_MANDATO],'''') ,nullif([IMPORTO_MANDATO],'''') ,nullif([NOTE],'''') FROM unscproduzione.dbo.SIGMA_TABELLA_SD02 where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY COD_LOCALE_MANDATO'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\' + @nomefile + ' -c -t"|" -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp
				...
				--dati
				set @sql = @sql + ' UNION SELECT nullif([COD_LOCALE_MANDATO],'''') ,nullif([NUMERO_MANDATO],'''') ,nullif([DATA_MANDATO],'''') ,nullif([IMPORTO_MANDATO],'''') ,nullif([NOTE],''''),idgenerazione FROM unscproduzione.dbo.SIGMA_TABELLA_SD02 where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY idgenerazione,COD_LOCALE_MANDATO'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\CSV\' + @nomefile + ' -c -t; -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp
			...
			--SD03
				...
				--dati
				set @sql = @sql + ' UNION SELECT nullif([COD_LOCALE_DOCUMENTO],'''') ,nullif([NOME_FILE],'''') ,nullif([TIPOLOGIA_FILE],'''') ,nullif([CATEGORIA_FILE],'''') ,nullif([COD_LOCALE_PROGETTO],'''') ,nullif([COD_LOCALE_MANDATO],'''') ,nullif([COD_SPESA_LOCALE],'''') ,nullif([NOTE],''''),idgenerazione FROM unscproduzione.dbo.SIGMA_TABELLA_SD03 where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY idgenerazione,COD_LOCALE_DOCUMENTO'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\CSV\' + @nomefile + ' -c -t; -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp
			...
			--SD04
				...
				set @sql = 'SELECT nullif([COD_LOCALE_MANDATO],''''),nullif([COD_LOCALE_SPESA],'''') FROM unscproduzione.dbo.SIGMA_TABELLA_SD04 where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY COD_LOCALE_MANDATO,COD_LOCALE_SPESA'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\' + @nomefile + ' -c -t"|" -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp
				...
				--dati
				set @sql = @sql + ' UNION SELECT nullif([COD_LOCALE_MANDATO],''''),nullif([COD_LOCALE_SPESA],''''),idgenerazione FROM unscproduzione.dbo.SIGMA_TABELLA_SD04 where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY idgenerazione,COD_LOCALE_MANDATO,COD_LOCALE_SPESA'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\CSV\' + @nomefile + ' -c -t; -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp
			...
			--SD06
				...
				set @sql = 'SELECT nullif([COD_LOCALE_SPESA],''''),nullif([COD_PAGAMENTO],'''') FROM unscproduzione.dbo.SIGMA_TABELLA_SD06 where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY COD_LOCALE_SPESA, COD_PAGAMENTO'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\' + @nomefile + ' -c -t"|" -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp	
				...
				--dati
				set @sql = @sql + ' UNION SELECT nullif([COD_LOCALE_SPESA],''''),nullif([COD_PAGAMENTO],''''),idgenerazione FROM unscproduzione.dbo.SIGMA_TABELLA_SD06 where idgenerazione = ' +  CONVERT(VARCHAR,@IdGenerazione) + ' ORDER BY idgenerazione,COD_LOCALE_SPESA, COD_PAGAMENTO'
				select @comandobcp = 'bcp "'+@sql+'" queryout \\Appl\modhelios$\SIGMA_FILES\CSV\' + @nomefile + ' -c -t; -T -CRAW -S' + @@servername
				exec master..xp_cmdshell @comandobcp	
