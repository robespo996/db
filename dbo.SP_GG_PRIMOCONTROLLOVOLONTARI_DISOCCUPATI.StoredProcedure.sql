USE [unscproduzione]
GO
/****** Object:  StoredProcedure [dbo].[SP_GG_PRIMOCONTROLLOVOLONTARI_DISOCCUPATI]    Script Date: 14/10/2025 12:36:28 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[SP_GG_PRIMOCONTROLLOVOLONTARI_DISOCCUPATI]
AS

IF EXISTS(select * from entit‡controlliDISOCCUPATI where data_elaborazione_controlli is null)
BEGIN
...
WHERE
 B.IDTIPOPROGETTO = 4 --progetto gg NEET
	and
	 c.Descrizione like '%DISOCCUPATI%'
	and  
	 ASA.StatoGraduatoria = 2 --volontario su graduatoria presentata non valutata
UNION --PER GRADUATORIE CONFERMATE DA RICONTROLLARE (ATTUALMENTE NON E' CONSENTITO DA APPLICAZIONE PER DISOCCUPATI)
SELECT A.IDENTIT‡,@data_richiesta_elaborazione, a.codicefiscale FROM ENTIT‡ A 
INNER JOIN GraduatorieEntit‡ GE ON A.IDEntit‡ = GE.IdEntit‡
INNER JOIN Attivit‡SediAssegnazione ASA ON GE.IdAttivit‡SedeAssegnazione = ASA.IDAttivit‡SedeAssegnazione
INNER JOIN ATTIVIT‡ B ON ASA.IDAttivit‡ = B.IDAttivit‡
