USE [unscproduzione]
GO
/****** Object:  StoredProcedure [dbo].[SP_TRASMETTIGRADUATORIAVOLONTARI_INVIOEMAILENTI]    Script Date: 14/10/2025 12:36:30 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[SP_TRASMETTIGRADUATORIAVOLONTARI_INVIOEMAILENTI]
		@IDENTE 		INT,
		@IDPROGRAMMA    VARCHAR(20),
...
begin try
	...
	select @codProgramma=isnull(CodiceProgramma,''), @titolo=titolo from programmi where idprogramma=@IDPROGRAMMA
	if @codProgramma='' 
	begin
		RAISERROR ('Il Codice Programma non e'' valorizzato',15,1)
	end
	...
	begin
		...
		set @dbname='unscproduzione.dbo.TrasmissioneGraduatorieDocumenti'
...
--		set @localPath = '\\Appl\modhelios$\COLLAUDO\TEST_MEV00014\'
		set @localPath = 'd:\ALLEGATI_TRASMISSIONE_GRADUATORIE\'
		set @dbname='SQLTEST.UnscEngCollaudo.dbo.TrasmissioneGraduatorieDocumenti'
		set @mailDestinat ='berardino.chirico@eng.it'
		set @COPIACARBONE = '' -- 'alinmihai.lefter@eng.it'
		set @COPIACARBONEN ='' 
	...
	@oldDatainvioemail varchar(500)

	if not exists (
		select 1
		from TrasmissioneGraduatorieDocumenti t1, TrasmissioneGraduatorieDocumentiProtocollo t2
		where t1.idDocumento=t2.idDocumento and t1.datainvioemail is null
		and idente=@idente and idprogramma=@IDPROGRAMMA and TipoDocumento='T')		
	begin

		select top 1 @oldDatainvioemail = isnull(convert(varchar,datainvioemail,103) + ' ' + convert(varchar,datainvioemail,108) ,'')
		from TrasmissioneGraduatorieDocumenti t1, TrasmissioneGraduatorieDocumentiProtocollo t2
		where t1.idDocumento=t2.idDocumento 
		and idente=@idente and idprogramma=@IDPROGRAMMA and TipoDocumento='T' and datainvioemail is not null
		order by datainvioemail desc

		if @oldDatainvioemail=''
		begin
			RAISERROR ('Non e'' stata trovata alcuna lettera di trasmissione da notificare',15,1)
		end 
		else begin
			set @oldDatainvioemail='La lettera di trasmissione e'' gia'' stato notificata in data ' + @oldDatainvioemail
			RAISERROR (@oldDatainvioemail,15,1)
		end
	end
	...
	if not exists (
		...
		from TrasmissioneGraduatorieDocumenti t1, TrasmissioneGraduatorieDocumentiProtocollo t2
		where t1.idDocumento=t2.idDocumento and t1.datainvioemail is null
		and idente=@idente and idprogramma=@IDPROGRAMMA and TipoDocumento='C')
	begin

		select top 1 @oldDatainvioemail = isnull(convert(varchar,datainvioemail,103) + ' ' + convert(varchar,datainvioemail,108) ,'')
		from TrasmissioneGraduatorieDocumenti t1, TrasmissioneGraduatorieDocumentiProtocollo t2
		where t1.idDocumento=t2.idDocumento 
		and idente=@idente and idprogramma=@IDPROGRAMMA and TipoDocumento='C' and datainvioemail is not null
		order by datainvioemail desc
		...
		begin
			...
			set @oldDatainvioemail='L''allegato C) e'' gia'' stato notificato in data ' + @oldDatainvioemail
			RAISERROR (@oldDatainvioemail,15,1)
		end
	end
	...
	from TrasmissioneGraduatorieDocumenti t1, TrasmissioneGraduatorieDocumentiProtocollo t2
	where t1.idDocumento=t2.idDocumento 
	and idente=@idente and idprogramma=@IDPROGRAMMA and TipoDocumento='T' and datainvioemail is null
	...
	from TrasmissioneGraduatorieDocumenti t1, TrasmissioneGraduatorieDocumentiProtocollo t2
	where t1.idDocumento=t2.idDocumento 
	and idente=@idente and idprogramma=@IDPROGRAMMA and TipoDocumento='C' and datainvioemail is null
	...
	SET @OGGETTO = 
'Trasmissione Graduatorie Ente ['+ @codiceEnte + '] e Allegato C  ñ Programma  ['+@titolo+'] (['+@codProgramma+'])'
...
relativi al programma ['+@titolo+'] (['+@codProgramma + ']) per l''ente ['+@codiceEnte+'] ñ ['+@denomEnte + ']</p>
...
' si chiede pertanto di non rispondere al messaggio.</p>'
	...
	if @rc=0
	begin
		update TrasmissioneGraduatorieDocumenti set DataInvioEmail=getDate()
	...
	update attivit‡sediAssegnazione set StatoGraduatoria=2 where IDAttivit‡SedeAssegnazione in (
		...
		FROM  attivit‡sediAssegnazione ASA with(nolock)
			...
			inner join GraduatorieEntit‡ GE with(nolock) on ASA.IDAttivit‡SedeAssegnazione=GE.IdAttivit‡SedeAssegnazione
			...
			inner JOIN Programmi with(nolock) on attivit‡.IdProgramma = Programmi.IdProgramma 
		WHERE --statiattivit‡.idstatoattivit‡ in (1,2)
			attivit‡.IDStatoAttivit‡ in (1,2)
			and programmi.IdEnteProponente=@idente
			and attivit‡.idprogramma=@idprogramma
			and ASA.statograduatoria=5   -- x DEROGA
		)

	insert into [TrasmissioneGraduatorieDocumentiNotificheEmail] 
	values (
		getDate(),
		@IdEnte,
		@IdProgramma,
...
print @return
	insert into [TrasmissioneGraduatorieDocumentiNotificheEmail] values (
		getDate(),
		@IdEnte,
		@IdProgramma,
		0,
