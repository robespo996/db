USE [unscproduzione]
GO
/****** Object:  StoredProcedure [dbo].[SP_OLP_DA_CONTROLLARE_2011]    Script Date: 14/10/2025 12:36:29 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[SP_OLP_DA_CONTROLLARE_2011] 
AS
BEGIN
	...
	WHERE     (attivit‡.IDStatoAttivit‡ IN (4, 5)) AND (entepersonaleruoli.IDRuolo = 1)
...
--			inner join bando f on e.idbando = f.idbando
			...
			where b.idruolo = 1 and d.idstatoattivit‡ in (4,5) and a.identepersonaleruolo = @IDENTEPERSONALERUOLO and i.maxvolontariperolp = 4)
...
--			inner join bando f on e.idbando = f.idbando
			...
			inner join macroambitiattivit‡ h on g.idmacroambitoattivit‡ = h.idmacroambitoattivit‡
			inner join iperambitiattivit‡ i on h.idiperambitoattivit‡ = i.idiperambitIattivit‡
			where b.idruolo = 1 and d.idstatoattivit‡ in (4,5) and a.identepersonaleruolo = @IDENTEPERSONALERUOLO 
--			where b.idruolo = 1 and f.gruppo = 17 and a.identepersonaleruolo = @IDENTEPERSONALERUOLO
	...
	if exists(select * from _Appo_Olp_TMP )
		EXEC  dbo.SSIS_sp_send_dbmail
			@profile_name = 'UNSC',
			@recipients = 'd.spagnulo@grupposerap.it;r.macioce@grupposerap.it',
